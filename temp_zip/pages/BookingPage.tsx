...
import { bookingService, roomService, paymentService } from '../src/services';
...
const BookingPage: React.FC = () => {
  ...
  const fetchRooms = useCallback(async () => {
    ...
    try {
      ...
      const message =
        (error as any).message || (error as any).response?.data?.message || 'Failed to load rooms. Please refresh the page.';
  ...
  React.useEffect(() => {
    if (!formData.roomId && !roomIdSetRef.current && rooms.length > 0) {
      setFormData(prev => ({ ...prev, roomId: rooms[0].roomId }));
  ...
  const createBooking = async (paymentDetails?: any) => {
    ...
    try {
      const bookingData = {
        ...
        paymentDetails: paymentDetails, // Store Razorpay details if any
      };
      ...
      setIsSubmitting(false);
      const errorMessage =
        (error as any).message || (error as any).response?.data?.message || 'Failed to create booking. Please try again.';
  ...
  const handleRazorpayPayment = async () => {
    ...
    try {
      ...
      const options = {
        key: import.meta.env.VITE_RAZORPAY_KEY_ID || 'your_key_id', // Should be in env
        amount: order.amount,
        currency: order.currency,
        name: 'Sai Lakshya Talkies',
        ...
        handler: async (response: any) => {
          try {
            ...
            showToast('Payment Verification Failed', 'There was an issue verifying your payment. Please contact support.', 'info');
          }
        },
        prefill: {
          name: formData.name,
      ...
      setIsSubmitting(false);
      showToast('Payment Initialization Failed', 'Could not start payment. Please try again or use manual payment.', 'info');
    }
  };
              ...
              {paymentMethod === 'online' ? (
                <>
                  <div className="relative group mb-8 flex justify-center">
                    <div className="bg-[#d4af37]/10 p-8 rounded-full border border-[#d4af37]/20">
                      <CreditCard className="text-[#d4af37]" size={64} />
                    </div>
                  </div>
    ...
    return (
      <div className="min-h-screen pt-32 pb-20 px-4 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-400 text-lg font-light">Loading rooms...</p>
        </div>
      </div>
    );
  }

  // If there was an error fetching rooms, show a retry option
  if (!loadingRooms && roomLoadError) {
    return (
      <div className="min-h-screen pt-32 pb-20 px-4 flex items-center justify-center">
        <div className="text-center max-w-md">
          <h2 className="text-2xl font-bold text-white mb-6">Unable to load rooms</h2>
          <p className="text-gray-400 mb-6">{roomLoadError}</p>
          <button
            onClick={() => fetchRooms()}
            className="px-8 py-3 bg-[#d4af37] text-black font-royal font-bold rounded-lg hover:bg-white transition"
          >
            Try Again
          </button>
        </div>
    ...
    return (
      <div className="min-h-screen pt-32 pb-20 px-4 md:px-6">
        <div className="max-w-4xl mx-auto text-center">
          <h1 className="font-royal text-4xl font-black gold-gradient uppercase tracking-widest mb-8">
            Select Your Royal Chamber
          </h1>
          <div className="grid md:grid-cols-2 gap-6">
            {rooms.map(room => (
              <div
                key={room.roomId}
                onClick={() => setFormData(prev => ({ ...prev, roomId: room.roomId }))}
                className="glass-card p-6 rounded-2xl border border-white/10 hover:border-[#d4af37]/50 transition-all cursor-pointer"
              >
                <h3 className="font-royal text-xl font-bold gold-gradient mb-2">{room.name}</h3>
                <p className="text-gray-400 text-sm mb-4">{room.description}</p>
                <p className="text-[#d4af37] font-bold">₹{room.price.toLocaleString()}</p>
              </div>
            ))}
          </div>
        </div>
